...
public class SearchTest<T extends Comparable<T>> {
    ...
    public boolean foundInRange(T a, T b){
        boolean start = false;
        boolean end = false;
    ...
    public int countInRange(T a, T b){
        int count = 0;
        for(int i = 0 ; i < value.length; i++){
            if(value[i].compareTo(a) > 0 && value[i].compareTo(b) < 0)
                count++;
            else
                continue;
        }
    ...
    public String listInRange(T a ,T b){
        String list = "";
        for(int i = 0 ; i < value.length; i++){
            if(value[i].compareTo(a) > 0 && value[i].compareTo(b) < 0)
                list = list + i +" ";
    ...
    public boolean binarySeachInRange(T a ,T b){
         int indexStart = binarySearchCount(0,value.length,a);
         int indexEnd = binarySearchCount(0, value.length,b);
    ...
    /*
    public boolean binarySearchCount(int start, int end, T a,T b){
          int middle = (start + end)/2;
          if(end < start){
              return false;
      }
          if((value[middle].compareTo(a) > 0 && orderType == ASCENDING) ||
              (value[middle].compareTo(b) < 0 && orderType == DESCENDING))
               return
   }
*/
}
